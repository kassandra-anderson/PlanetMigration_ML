...
df_train = pd.read_pickle(path_to_data + "train_data_seed_" + str(seed) + "_testsize_" + testsize + ".pkl")
ylabel = "flag"

# Group features by different categories

# These are the raw input variables (mostly orbital elements)
orb_features = ["m2_input",
                "m3_input",
                "a1_input",
                "a2_input",
                "e2_input",
...
emax_features = ["log_peri_max_over_rtide_input",
                 "log_aF_max_input",
                 "log_aF_max7_input"
                ]

# Powers of aF = a(1 - emax^2)
emax_powers_features = ["log_peri_max_over_rtide_input",
                        "log_aF_max_input",
                        "log_aF_max2_input",
                        "log_aF_max3_input",
                        "log_aF_max4_input",
...
feature_dict = {"inputvars":orb_features,
...
params_RF = {"RF__n_estimators": randint(10,100),
...
for feature_name, features, in feature_dict.items():
    ...
    pipeline = imbl_pipeline([("scaler",scaler), ("resampler",resampler), (name,ml_model)])
    print("Made pipeline")

    # Fit model and tune hyperparameters
    print("Tuning model...")
    model_cvs[name] = tts.train_model(X_train,
                                      y_train,
                                      pipeline,
                                      model_params,
                                      cv,
    ...
    # Pickle the pipeline
    outfile = outpath + name + "_" + scaler_name + "_" + resampler_name + "_features_" + feature_name + \
              "_seed_" + str(seed) + "_testsize_" + testsize + "_cv_nsplits_" + str(n_splits) + ".pkl"
